...
RandGigit = random.choice(Digits)
RandUpChar = random.choice(UpCaseChar)
RandlwChar = random.choice(LwCaseChar)
RandSpSymbol = random.choice(SplSymbol)

temp_pass = RandGigit + RandUpChar + RandlwChar + RandSpSymbol
...
try:
	...
	if Maxlen >=4:
		...
		print("Enter above 4 length.")
		
except:
	print("Enter Digit only without fractional ")
print(password)